...
function CreateAdminPanel(){
    ...
    for (let i = 0; i < AdminTable.length; i++) {
        $("#admin_menu ul").append(`<li><button class="dropdown-item" id="admin_hq_${i}" onclick="SetAdminDatas(id, ${i})">${AdminTable[i].name == ""?'EMPTY':AdminTable[i].name}<br><span>${AdminTable[i].hq} | Pos: ${Math.round(AdminTable[i].pos.x)}, ${Math.round(AdminTable[i].pos.y)}, ${Math.round(AdminTable[i].pos.z)}</span></button></li>`)
    }
