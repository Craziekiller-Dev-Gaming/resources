...
Config.Settings = {
    ...
    lighter = {
        ...
        enableFluidUse = false, -- If fluid metadata should be changed & checked when lighting, note that your item has to be unique (not stackable)
    ...
    ["cigar"] = {
        ...
        effects = {
            ...
            clientFunc = function(value, item, metadata)
                -- print("Total amount has been removed:", value)
                -- print("Used item", item)
                TriggerServerEvent('hud:server:RelieveStress', 4)
                -- print("Raw data received:", json.encode(metadata))
            end,
        },
    },
